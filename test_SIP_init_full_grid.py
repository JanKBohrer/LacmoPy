    ...
    for i in range(no_cells[0]):
        ...
        for j in range(no_cells[1]):
            ...
            for mode_n in range(no_modes):
                ...
                mask_mode = np.logical_and(mask_mode, mask_ij)
                moments_num[i,j,0,mode_n] = np.sum( xi[mask_mode] ) / dV
...
moments_an = np.load(save_path + "moments_an.npy")
moments_num = np.load(save_path + "moments_num.npy")

moments_num_norm = np.zeros_like(moments_num)

for i in range(no_cells[0]):
    for j in range(no_cells[1]):
        moments_num_norm[i,j] = moments_num[i,j]/moments_an[j]

#%%

import matplotlib.pyplot as plt
fig, axes = plt.subplots(nrows=no_modes)

for mode_n in range(no_modes):
    ax = axes[mode_n]
    for n in range(4):
        ax.plot(n*np.ones_like(moments_num_norm[:,:,n,mode_n]).flatten(),
                moments_num_norm[:,:,n,mode_n].flatten(), "o")
