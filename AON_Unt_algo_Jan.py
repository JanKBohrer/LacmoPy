...
### THIS IS THE MOST RECENT VERSION OF MY ALGORITHM 
### IMPLEMENTED DIRECTLY FROM THE PAPER OF UNTERSTRASSER
...
def collision_step_Long_Bott_kernel_grid_np(
    ...
    for i in range(no_SIPs):
        ...
        ind_kernel[i] = \
            ( masses_log[i] - m_kernel_low_log ) / bin_factor_m_log + 0.5
        if ind_kernel[i] < 0:
            ind_kernel[i] = 0
...
#            p_crit = xi_max * kernel_Long_Bott(m_min, m_max) * dt/dV
            p_crit = xi_max * kernel_grid[ind_kernel[i], ind_kernel[j]] * dt_over_dV
            ...
            if p_crit > 1.0:
                ...
                ind_kernel[ind_min] = \
                    ( np.log(masses[ind_min]) - m_kernel_low_log ) \
                    / bin_factor_m_log + 0.5
                if ind_kernel[ind_min] < 0:
                    ind_kernel[ind_min] = 0
                elif ind_kernel[ind_min] > (no_kernel_bins - 1):
                    ind_kernel[ind_min] = no_kernel_bins - 1
                # print(j,i,p_crit)
            elif p_crit > rnd[cnt]:
                xi_rel_dev = (xi_max-xi_min)/xi_max
                if xi_rel_dev < 1.0E-5:
                    ...
                    xis[ind_max] = 0.5 * 0.7 * xi_ges
                    xis[ind_min] = 0.5 * xi_ges - xis[ind_max]
                    # mass of ind_min AND ind_max changed -> update kernel ind:
                    ind_kernel[ind_min] = \
                        ( np.log(masses[ind_min]) - m_kernel_low_log ) \
                        / bin_factor_m_log + 0.5
                    if ind_kernel[ind_min] < 0:
                        ...
                        ind_kernel[ind_min] = no_kernel_bins - 1
                    ind_kernel[ind_max] = \
                        ( np.log(masses[ind_max]) - m_kernel_low_log ) \
                        / bin_factor_m_log + 0.5
                    if ind_kernel[ind_max] < 0:
                        ind_kernel[ind_max] = 0
                    elif ind_kernel[ind_max] > (no_kernel_bins - 1):
                        ind_kernel[ind_max] = no_kernel_bins - 1
                else:
                    masses[ind_min] += m_max
                    xis[ind_max] -= xi_min
                    # mass of ind_min changed -> update kernel index:
...
#################################################
### NOW: plan for 2D dynamic simulation: 
# masses in 1E-18 kg
# radii in 1E-6 m
# assuming 
one_third = 1.0/3.0
def collision_step_Long_Bott_effi_grid_np(
    xis, masses, radii, velocities, mass_densities,
...
#    masses_log = np.log(masses)
    ...
    dv = np.zeros((no_SIPs*(no_SIPs-1))//2, dtype = np.float64)
    cnt = 0
    for i in range(0,no_SIPs-1):
        v_i = velocities[i]
        for j in range(i+1, no_SIPs):
            dv[cnt] = velocities[j] - v_i
            cnt += 1
    dv = np.abs(dv)
    ...
    for i in range(no_SIPs):
        # auto conversion to int due to dtype of ind_kernel
        ind_kernel[i] = \
            ( math.log(radii[i]) - R_kernel_low_log ) / bin_factor_R_log + 0.5
        if ind_kernel[i] < 0:
            ind_kernel[i] = 0
        elif ind_kernel[i] > (no_kernel_bins - 1):
            ind_kernel[i] = no_kernel_bins - 1
...
#            p_crit = xi_max * kernel_grid[ind_kernel[i], ind_kernel[j]] * dt_over_dV
            ...
            if p_crit > 1.0:
                ...
                masses[ind_min] = (xi_min*m_min + xi_col*m_max) / xi_min
                xis[ind_max] -= xi_col
                radii[ind_min] = compute_radius_from_mass(masses[ind_min],
                ...
                # print(j,i,p_crit,rnd[cnt])
            cnt += 1
collision_step_Long_Bott_effi_grid = \
    njit()(collision_step_Long_Bott_effi_grid_np)
...
def simulate_collisions_kernel_grid(
        xis, masses, dt, dV, t_end, dt_save, seed, save_dir, kernel,
        kernel_grid, m_kernel_low, bin_factor_m):
    if kernel == "Golovin":
        collision_step = collision_step_Golovin
    elif kernel == "Long_Bott":
        collision_step = collision_step_Long_Bott_kernel_grid
#        collision_step = collision_step_Long_Bott
    np.random.seed(seed)
    # save_path = save_dir + f"seed_{seed}/"
    # t = 0.0
    ...
    dt_over_dV = dt/dV
    no_kernel_bins = kernel_grid.shape[0]
    m_kernel_low_log = np.log(m_kernel_low)
    bin_factor_m_log = np.log(bin_factor_m)
