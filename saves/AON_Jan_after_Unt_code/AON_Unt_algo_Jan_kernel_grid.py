...
incl_path = "/home/jdesk/CloudMP/CodeCompute_Unt"
import sys
...
# NOTE that this Kernel is tabulated in his version -> this fct is not
# nec. anymore -> see algo below
    
# i.e. the values are calculated on a fine radius grid,
# then converted to mass and then tabulated on a log mass grid
# there is NO INTERPOLATION done inside the mass grid.
# the grid values are just centered
...
# eta_indize = m(i+1)/m(i) = const, where m(i) is the kernel mass grid
def collision_step_Long_Bott_np(
    xis, masses, dt_over_dV,
    cck, m_kernel_low, eta_indize):
#dt             : Zeitschritt in s
#cck            : Kernel-Matrixwerte
#nr_SIPs        :
#xis    : Vektor mit SIP-Gewichtsfaktor (will be multiplied by skal_n in KCE)
#masses    : Vektor mit SIP-Troepfchenmassen (will be multiplied by skal_m in KCE)
#m_kernel_low          : kleinste Masse fuer die Kernelwert in Matrix gegeben ist
#eta_indize     : Anzahl der Kernelwerte in Matrix pro Massendekade
#m_kernel       : mass grid on which kernel values are given
#dV             : Volumen der Gitterbox in m^3
...
#    ibreak = 0
    ...
    no_SIPs = xis.shape[0]
    mass_ind=np.zeros(no_SIPs, dtype = np.int64)
    for i in range(0,no_SIPs):
        if(math.log(masses[i])>math.log(m_kernel_low)):
            mass_ind[i] = \
                math.floor( ( math.log(masses[i]) - math.log(m_kernel_low) ) 
                            / math.log(eta_indize) + 0.5 )
            if mass_ind[i] < 0: mass_ind[i] = 0
            elif mass_ind[i] > 398: mass_ind[i] = 398
    #print(min(indize),max(indize),nr_SIPs)
#    np.clip(mass_ind,0,398,out=mass_ind)
#    np.clip(indize,0,398,out=indize)
# floor liefert float values, in int umwandeln, damit die Werte als Indizes verwendet werden koennen
...
#            iju=indize_floored[j]
            cck_value=cck[iiu,iju]*dt_over_dV
            nEK_agg_tmp=cck_value*xis[i]*xis[j]   # eigentlich linke mit skal_n und rechte Seite mit skal_n^2 multiplizieren
            mEK_agg_tmp=masses[i]+masses[j]

            tmp_min=min(xis[i],xis[j])
            if(tmp_min!=0):
...
#                    count_colls[1] += 1
                    if(xis[i]<xis[j]):
                        ...
                        i1=j; i2=i
                    xis[i2]=xis[i2]-xis[i1]
                    masses[i1]=mEK_agg_tmp
#                else:
#                    count_colls[0] += 1
            elif (c_mcCEIL > 1):
            ...
            #Floating Point Multiple Collection, das berechnete c_mc wird verwendet, diese Operation generiert nicht-ganzzahlige Vielfache der Ausgangsmassen
                masses[i1]=(masses[i1]+c_mc*masses[i2])
                xis[i2]=xis[i2]-xis[i1]*c_mc
...
######################### MY ALGO WITH CALC KERNEL FOR EACH PAIR
...
def simulate_collisions(xis, masses, dt, dV, t_end, dt_save, seed, save_dir,
                        kernel, cck, m_kernel_low, eta_indize):
    dt_over_dV = dt/dV
    if kernel == "Golovin":
    ...
    for step_n in range(no_steps):
        ...
        collision_step(xis, masses, dt_over_dV, cck, m_kernel_low, eta_indize)
        # t += dt
    t = (step_n+1) * dt
    xis_vs_time[save_n] = np.copy(xis)
    masses_vs_time[save_n] = np.copy(masses)
...
### NOT NEC: AON Method with Algo from Unterstrasser NEW with tabulated kernel


#masses = np.logspace(-18,-10,10)
#xis = np.linspace(1.0E8,1.0E9,10)
#
#dt = 10.0
#
#dV = 1.0
...
#m_kernel_low=m_kernel[0]/eta_indize
#
#masses0 = np.copy(masses)
#xis0 = np.copy(xis)
#
#collision_step_Long_Bott(
#    xis, masses, dt_over_dV, cck, m_kernel_low, eta_indize)
#
#print(xis0)
#print(xis)
#print(xis-xis0)
#print(masses-masses0)
#print( (np.sum(xis*masses) - np.sum(xis0*masses0))/(np.sum(xis0*masses0)) )


#%% AON Method with Algo from Unterstrasser
...
### CHANGED with resp. to Unterstrasser -> I think it is correct this way..
m_kernel_low=m_kernel[0]
#m_kernel_low=m_kernel[0]/eta_indize

if OS == "MacOS":
    sim_data_path = "/Users/bohrer/sim_data_strict_threshold/"
#    sim_data_path = "/Users/bohrer/sim_data_after_Unt/"
#    sim_data_path = "/Users/bohrer/sim_data/"
elif OS == "LinuxDesk":
#    sim_data_path = "/mnt/D/sim_data/"
    sim_data_path = "/mnt/D/sim_data_strict_threshold_kernel_grid_m_low_shift/"
...
if simulate:
    for kappa in kappa_list:
        ...
        # LINUX desk
        ensemble_dir =\
        "/mnt/D/sim_data_strict_threshold/" + f"col_box_mod/ensembles/{dist}/{gen_method}/eta_{eta:.0e}/kappa_{kappa}/"
#        sim_data_path + f"col_box_mod/ensembles/{dist}/{gen_method}/eta_{eta:.0e}/kappa_{kappa}/"
        ...
        for cnt,seed in enumerate(seed_list):
            ...
            simulate_collisions(xis, masses, dt, dV, t_end, dt_save, seed,
                                save_dir, kernel, cck, m_kernel_low, eta_indize)
...
if plot_moments_kappa_var:
    ...
    fig_name += f"_dt_{int(dt)}_no_sims_{no_sims}_sseed_{start_seed}.png"
    no_rows = 4
    ...
    if kernel == "Golovin":
        ...
        axes[0].set_yticks( np.logspace(5,8,4) )
#        axes[0].set_yticks([1.0E6,1.0E7,1.0E8])
        axes[0].set_ylim([1.0E5,4.0E8])
