c runtergeladen von http://www2.meteo.uni-bonn.de/forschung/gruppen/tgwww/people/index_english.php?memid=2&page=fortran.html
c start the programm with the following commands
c ifort Fluxmethod_Bott.f -o Fluxmethod_Bott.o
c ./Fluxmethod_Bott.o
      ...
      data emin,pi,tmax/1.e-9,3.141592654,3600./ ! emin in mg, tmax in s
...
c       scal = number of bins after which the mass doubles
...
c      rq0 = 10.		 ! Bott_Init
      ...
      scal = ##scal##
      isw =  ##kernel##
      rq0=rq0*1.e-04 ! umrechnen in cm
      xmw=xmw*1.d-3  ! umrechnen in kg/m**3
      xn0=4./3.*pi*1000.*exp(log(rq0)*3.)  ! mean mass  in mg: kg/m^3 * cm^3= 1e-6*kg = mg

      xn1=xmw/xn0  ! total number concentration cm^-3
	 write (6,*) 'mean mass, number concentration',xn0,xn1
      dlnr=dlog(2.d0)/(3.*scal)
      ax=2.d0**(1.0/scal) 
      write (6,*) 'scal,ax,dlnr',scal,ax,dlnr

c mass and radius grid in mg und um
      e(1)=emin*0.5*(ax+1.)
...
c initial mass distribution
      x0=xn1/xn0  ! number concentration in cm^-3 durch mean mass mg  mean LWC in 
      do i=1,n
         x1=e(i)  ! mg 
         g(i)=3.*x1*x1*x0*dexp(-x1/xn0)
c        obige Zeile ist Formel 21 aus Berry: x0 oben ist L/(x_f0)^2 nach Berry x1 = x. Die Normierung um auf s zu kommen, steckt dann schon in x0.
c        g ist dann wie oben schon angegeben in mg/cm^3
      enddo
      rmaxg=maxval(g)
      write (6,*) 'e(n) in mg,r(n) in  um ', e(n),r(n),g(n)
      write (6,*) rmaxg, 'g(n)/gmaxinit ',g(n)/rmaxg
...
c output for plotting
      do i=1,n
         ...
         eei(i)=min(1.d38,e(i))
      enddo
c      open (16,file='boplot00_Golovinkernel_scal1.out') !,status='old')
c      open (16,file='boplot00_Golovinkernel.out') !,status='old')
c      open (16,file='boplot00_hydrodynamickernel.out') !,status='old')

      if (isw.eq.0) open (16,file='g_Long.out')
      if (isw.eq.1) open (16,file='g_Hall.out')
      if (isw.eq.2) open (16,file='g_Golovin.out')
      
      write (16,*) n,dt*nt/200+1     !Anzahl der Bins und ausgegeben Zeitpunkte

      write (16,6100) rri,eei  ! Radius grid in um und mass grid in mg
 6100 format (5E16.7E3)
...
c multiply kernel with constant timestep and logarithmic grid distance
      ...
      write (16,*) 0.0
      write (16,6100) g !in mg/cm^3 per ln r
...
c collision
         call coad 
c output for plotting
        ! print *,'time in seconds:',tlmin,mod(tlmin, 200.)
         if (mod(tlmin, 200.).eq.0) then
            !tlmin=tlmin-60.
            !lmin=lmin+1
            print *,'time in seconds:--------------------',tlmin
            write (16,*) t
            write (16,6100) g   !in mg/cm^3 per ln r
c mass balance
            x0=0.
     ...
     .       'rmax', r(imax),'g(n)/gmaxinit',g(n)/rmaxg
         endif
      enddo
      close (16)
      stop 'stop coad1d'
...
c         ! das ist unglücklich ausgedrückt: die Kernelwerte werden lediglich geglättet.
c         ! Der Kernel wird sicherlich nicht an bestimmte Stelle durch Interpolation ausgewertet!
      do i=1,n
      do j=1,n
         jm=max0(j-1,1)
         im=max0(i-1,1)
     ...
     &           +.5*cck(i,j)
      ...
      subroutine fallg
c terminal velocity of falling drops in cm/s wahrscheinlich!
      parameter (n=##nbin##)
      implicit double precision (a-h,o-z)
      common /grid/ g(n),r(n),e(n)
      common /kern/ ck(n,n),ec(n,n)
