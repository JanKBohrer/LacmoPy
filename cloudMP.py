...
from evaluation import sample_masses, sample_radii, compare_functions_run_time
...
for cnt,t in enumerate(np.arange(t_start, t_end, dt)):
    ...
    if cnt % dump_every == 0:
        dump_particle_data(t, pos, vel, m_w, m_s, xi, grid.temperature,
                           grid.mixing_ratio_water_vapor, path)
...
####################### CHECK
    ...
    delta_r_v_ad = -2.0 * dt_sub * compute_divergence_upwind(grid, grid.mixing_ratio_water_vapor,
                                                       flux_type = 1, boundary_conditions = [0,1]) \
                    * grid.rho_dry_inv - delta_r_v_ad

    delta_Theta_ad = -2.0 * dt_sub * compute_divergence_upwind(grid, grid.potential_temperature,
                                                     flux_type = 1, boundary_conditions = [0,1]) \
