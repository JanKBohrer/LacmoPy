...
def dst_lognormal_m2_np(m, mu_m_log, sigma_m_log):
    return np.exp( -0.5*( ( np.log( m ) - mu_m_log ) / sigma_m_log )**2 ) \
           / (m * two_pi_sqrt * sigma_m_log)
dst_lognormal_m2 = njit()(dst_lognormal_m2_np)
...
dist_par_m2 = (mu_m_log, sigma_m_log)

R_ = np.logspace(-6,4,10000) 
...
f_m_ana2 = dst_lognormal_m2(m_, *dist_par_m2)
...
m_mean_R = num_int_lognormal_mean_mass_R(R_[0], R_[-1], dist_par_m)
...
ax = axes[3]
ax.plot(R_, (f_m_ana2 - f_m_ana) /f_m_ana )
ax.set_xscale("log")
