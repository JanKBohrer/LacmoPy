...
#
#@njit()
#def compute_equilibrium_saturation_mf_AS(mass_fraction_solute_,
#                                     mass_solute_,
#                                      temperature_,
#                                      mass_density_particle_,
#                                      surface_tension_):
...
#@vectorize("float64(float64,float64)")
#def compute_surface_tension_AS_Sven(w_s, T):
#    return compute_surface_tension_water(T) \
...
#par_NaCl_Sven = 1.62E-3           
#def compute_surface_tension_NaCl_Sven(molality, temperature):
#    return compute_surface_tension_water(temperature)/0.072 \
#           * (0.072 + par_NaCl_Sven * molality )
...
sigma_AS = mp.compute_surface_tension_AS_Prup(w_s, T)

molal = mp.compute_molality_from_mass_fraction(w_s, c.molar_mass_AS*1E-3)

sigma_AS_Sven = mp.compute_surface_tension_AS_mol_Sven(molal, T)
sigma_AS_Sven2 = mp.compute_surface_tension_AS(w_s,T)
sigma_NaCl_Sven = mp.compute_surface_tension_NaCl_mol_Sven(molal, T)

#

a_w = mp.compute_water_activity_AS(w_s)

kelvin_term_AS = mp.compute_kelvin_term_mf(w_s, T, m_s, rho_p, sigma_AS)
kelvin_term_AS_Sven = mp.compute_kelvin_term_mf(w_s, T, m_s, rho_p, sigma_AS_Sven2)
kelvin_term_AS_w = mp.compute_kelvin_term_mf(w_s, T, m_s, rho_p, sigma_w)

S_eq = \
    mp.compute_equilibrium_saturation_mf_AS(w_s,
                                     m_s,
                                     T,
                                     rho_p,
                                     sigma_AS)
...
m_s_NaCl = mp.compute_mass_from_radius_jit(R_s, c.mass_density_NaCl_dry)

m_p_NaCl = m_s_NaCl/w_s
R_p_NaCl = mp.compute_radius_from_mass_jit(m_p_NaCl, rho_NaCl)

m_w_NaCl = m_p_NaCl - m_s_NaCl

a_w_NaCl = mp.compute_water_activity(m_w_NaCl, m_s_NaCl, w_s)

kelvin_term_NaCl_Sven = mp.compute_kelvin_term(R_p_NaCl, T,
                                                rho_NaCl, sigma_NaCl_Sven)
kelvin_term_NaCl_w = mp.compute_kelvin_term(R_p_NaCl, T,
                                                rho_NaCl, sigma_w)

S_eq_NaCl = mp.compute_equilibrium_saturation_NaCl_mf(w_s, T, m_s_NaCl)

S_eq_NaCl_sven = a_w_NaCl * mp.compute_kelvin_term(R_p_NaCl, T,
                                                rho_NaCl, sigma_NaCl_Sven)
...
R_p_NaCl_exp = mp.compute_radius_from_mass_jit(m_p_exp, rho_NaCl_exp)
...
for n in range(no_rows):
    axes[n].plot(R_p, data[n], label = "AS Prup")
    axes[n].set_ylabel(data_string[n])

axes[0].plot(R_p_NaCl, rho_NaCl, label="NaCl")    
...
#axes[1].legend()  
axes[2].plot(R_p_NaCl, a_w_NaCl, label = "NaCl")    
axes[2].plot(R_p_NaCl_exp, a_w_exp, label= "exper.")    

axes[3].plot(R_p_NaCl, kelvin_term_AS_w, label = "AS water")    
axes[3].plot(R_p_NaCl, kelvin_term_AS_Sven, label = "AS Sven")    
axes[3].plot(R_p_NaCl, kelvin_term_NaCl_w, label = "NaCl water")    
axes[3].plot(R_p_NaCl, kelvin_term_NaCl_Sven, label = "NaCl Sven")    
#axes[3].legend()
axes[4].plot(R_p, S_eq_AS_sigma_w, label = "AS sigma_w")    
axes[4].plot(R_p, S_eq_AS_Sven, label = "AS sigma_Sven")    
...
#                                      surface_tension_):
fig.tight_layout()
fig.savefig(f"solute_compare/S_eq_a_w_sigma_vs_R_p_for_D_s_{D_s*1E3:.0f}")

#%%
no_rows = 1
fig, axes = plt.subplots(no_rows, figsize=(10,6*no_rows))
axes.plot(R_p_NaCl, (S_eq_NaCl-S_eq_NaCl_sven)/S_eq_NaCl_sven)
axes.plot(R_p_NaCl, (kelvin_term_NaCl_w-kelvin_term_NaCl_Sven)/kelvin_term_NaCl_Sven)

#%%
no_rows = 1
fig, axes = plt.subplots(no_rows, figsize=(10,6*no_rows))
axes.plot(R_p, sigma_AS_Sven)
axes.plot(R_p, sigma_AS_Sven2)
axes.plot(R_p, sigma_AS)
...
D_s_list = np.array([6.,8,10,20,40,60])

D_s_list *= 1E-3

for n, D_s in enumerate(D_s_list):
    ...
    sigma_w = mp.compute_surface_tension_water(T)
    sigma_AS = mp.compute_surface_tension_AS(w_s, T)
    ...
    ax = axes[n]
    ax.plot(S_eq, R_p/R_s, label=f"{D_s*1E3:.0f}")
    ax.grid()
    ax.legend()
...
#sigma_NaCl = atm.compute_surface_tension_water(w_s, T_p)    
#
#gamma_AS = mp.compute_mass_rate_AS(m_w, m_s, w_s, R_p, T_p, rho_p,
